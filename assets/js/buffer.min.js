...
function loginWithWalletNameHash(name,web3) {
  // Reject empty names:
  var node = '0x'
  for (var i = 0; i < 32; i++) {
    node += '00'
  ...
  if (name) {
    ...
    for(var i = labels.length - 1; i >= 0; i--) {
      var labelSha = web3.utils.keccak256(labels[i])
			var str = node + labelSha.substr(2);
			node = web3.utils.keccak256(new Buffer(node.substr(2) + labelSha.substr(2), 'hex'));
    }
  }
